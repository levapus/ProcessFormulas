...
//  val back = GraphFrame.fromGraphX(gx)
  ...
  val preprocessedGx = gx.mapVertices((id, row) =>
    if(row.getString(0) == "r00t")
      (row.getString(0), row.getString(2), 0D, 0D)
    else
      (row.getString(0), row.getString(2), Double.NegativeInfinity, Double.PositiveInfinity))
  ...
  val processedGx = preprocessedGx.pregel(
    //initial message
    initialMsg,
//    ("", Double.PositiveInfinity, Map[String, Double]()),
    ...
    (id, vAttr, msg) => {
      ...
      if(msg._1 == Double.NegativeInfinity) {
        ...
        val init_vAttr = (vAttr._1, vAttr._2, math.max(vAttr._3, msg._1), vAttr._4)
...
//        val a: Option[() => Int] = ArithmeticParser.readExpression(formula)
//        (vAttr._1, formula, math.max(vAttr._3, msg._1), a.get())
        val new_vAttr = (vAttr._1, formula, math.max(vAttr._3, msg._1), vAttr._4)
        println(s"  set new values: $new_vAttr")
        new_vAttr
      }
//      (vAttr._1, vAttr._2, math.max(vAttr._3, msg._1), vAttr._4)
    },
    //sendMsg - send Message - send the value to vertices
    triplet => {
      if (triplet.srcAttr._3 != Double.PositiveInfinity) {
        println(s"sendMsg from ${triplet.srcAttr._1} to ${triplet.dstAttr._3}: " + (triplet.dstId, (triplet.srcAttr._3 + 1, Map(triplet.srcAttr._1 -> triplet.srcAttr._4))))
        Iterator((triplet.dstId, (triplet.srcAttr._3 + 1, Map(triplet.srcAttr._1 -> triplet.srcAttr._4))))
      } else {
        Iterator.empty
      }
    },
